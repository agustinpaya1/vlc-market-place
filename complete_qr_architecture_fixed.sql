-- Script COMPLETO CORREGIDO para implementar la nueva arquitectura de QR codes
-- IMPORTANTE: Ejecutar en Supabase Dashboard en este orden
-- NOTA: Usamos la tabla 'keys' en lugar de 'keys_public' (que es una vista)
...
-- ========================================
-- PASO 2: CONECTAR qr_codes CON keys
-- ========================================

-- Agregar columna key_id para conectar con keys (tabla real)
ALTER TABLE qr_codes 
ADD COLUMN IF NOT EXISTS key_id UUID REFERENCES keys(id);
...
-- Índices para conectar con keys
...
-- Habilitar RLS en keys (tabla real)
...
SELECT 
    ...
    cmd
FROM pg_policies 
WHERE tablename IN ('keys', 'qr_codes')
...
BEGIN
    ...
    RAISE NOTICE '✅ Tablas conectadas: qr_codes ↔ keys';
    ...
    RAISE NOTICE '⚠️ NOTA: keys_public es una vista, usando tabla keys directamente';
END $$;
