-- Script para probar la recompensa de VLcoins con datos reales
...
BEGIN
    ...
    -- 4. Guardar el estado actual para poder revertirlo
    ...
    -- 8. Revertir los cambios para no afectar datos reales
    ROLLBACK TO SAVEPOINT before_test;
    RAISE NOTICE 'Cambios revertidos - El pedido ha vuelto a estado "pending"';
    
    -- 9. Verificar que se haya restaurado el balance original
    SELECT COALESCE(balance, 0)
    INTO new_vlcoin_balance
    FROM vlcoin
    WHERE user_id = test_user_id;
    
    IF new_vlcoin_balance = initial_vlcoin_balance THEN
        RAISE NOTICE '✅ Balance restaurado correctamente: % VLcoins', new_vlcoin_balance;
    ELSE
        RAISE NOTICE '❌ Error al restaurar el balance';
    END IF;
    ...
    RAISE NOTICE 'La prueba se realizó satisfactoriamente y los cambios fueron revertidos';
END $$; 